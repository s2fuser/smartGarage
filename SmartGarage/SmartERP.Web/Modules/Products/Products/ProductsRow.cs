...
namespace SmartERP.Products.Entities
{
    ...
    public sealed class ProductsRow : SmartERP.LoggingRow<ProductsRow.RowFields>, IIdRow, INameRow
    {
        ...
        [DisplayName("Barcode"), Size(250), QuickSearch]
        ...
        [DisplayName("Product Name"), Size(250),LookupInclude, NameProperty]
        ...
        [DisplayName("Qty Out"), Column("QtyOut"), Expression("((SELECT SUM(sd.[Quantity])  FROM [dbo].[SaleDetails] sd where sd.[ProductId]=t0.[Id]) + (SELECT SUM(sd.[Quantity])  FROM [dbo].[InvoiceItems] sd where sd.[ProductId]=t0.[Id]))"), LookupInclude]
        ...
        [DisplayName("Stock"), Column("Stock"), Expression("((SELECT SUM(pd.[Quantity])  FROM [dbo].[PurchaseDetails] pd where pd.[ProductId]=t0.[Id]) - (SELECT SUM(sd.[Quantity])  FROM [dbo].[SaleDetails] sd where sd.[ProductId]=t0.[Id]) - (SELECT SUM(sd.[Quantity])  FROM [dbo].[InvoiceItems] sd where sd.[ProductId]=t0.[Id]))"), LookupInclude]
        public double? Stock
        {
